...
export default function () {
    ...
    group('Manage Address', function () {
        ...
        if (allAddressesGet.json().length > 0) {
            let index = generateRandomIndex(allAddressesGet.json().length - 1)
            addressId = allAddressesGet.json()[index].addressId
        }
        ...
        const deleteAddress = http.del(`${baseUrl}/users/v1/profile/address/${addressId}`);
        check(deleteAddress, {
