...
function makePurcahse() {
    ...
    let getAllCategoriesResponse = http.get('/catalog/v1/products/get-all-categories?productLimit=10&limit=100');

    check(getAllCategoriesResponse, {
        'GET All Categories Status is 200': (res) => res.status === 200,
    });

    let customerId = 'replace-with-customer-id';
    let getAssistedResponse = http.get(`/catalog/v1/assisted/${customerId}`);

    check(getAssistedResponse, {
        'GET Assisted Status is 200': (res) => res.status === 200,
    });
